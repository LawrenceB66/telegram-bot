import os
import requests
BOT_TOKEN = os.getenv("TELEGRAM_TOKEN") CHAT_ID = os.getenv("CHAT_ID")
def _ordinal(number): number = int(number)
if 10 <= number % 100 <= 20:
    suffix = "th"
...
def _alert_descriptor(signal_name): if not signal_name: return ""
name = str(
    signal_name
).strip().upper()
...
return ""
def send_alert( symbol, price, change_pct, signal, rvol=None, participation_pct=None, ): try: if signal is None: print( f"SKIPPING — SIGNAL IS NONE: {symbol}" ) return
